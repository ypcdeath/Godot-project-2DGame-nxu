...
@onready var defualt_player_stats := player_stats.to_dict()
...
func new_game() -> void:
	change_scene("res://worlds/forest.tscn", {
		...
		init = func():
			world_states = {}
			player_stats.from_dict(defualt_player_stats)
	})

func back_to_tile() -> void:
	change_scene("res://title_screen.tscn",{
		duration = 0.2,
	})
